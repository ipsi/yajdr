...
public class WorldOfDarknessGui extends JPanel implements ActionListener, ThreadProgressListener<WorldOfDarknessThreadInformation>
{
	...
	private void showResult(int numSuc, int numBotch, String dieResult)
	{
		...
		/* This should be the norm. The roll can be botched, and ones cancel successes. */
		...
		/* This should be reached when the roll cannot be botched, but ones do cancel successes. */
		...
		/* This should be reached when the roll cannot be botched, and ones don't cancel. */
		else if (!botchable.isSelected() && !onesCancel.isSelected())
		{
			if (numSuc >= 1)
			{
		...
		/* This should only be reached when the roll can be botched, but ones don't cancel. */
		else
		{
			if (numSuc == 0 && numBotch >= 1)
			{
